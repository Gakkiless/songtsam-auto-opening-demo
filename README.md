...
5. 产品运营可在“规则配置”页签调整业务类型频次、出发日限制、首选/次选出发日、库存预占、房型策略和产品默认配置。
6. 系统先读取业务类型默认规则，再叠加产品/行程覆盖配置。
7. 系统按日期区间生成候选出发日。
...
配置入口在页面顶部的“规则配置”页签。当前是 Demo 本地状态，修改后会立即影响本次生成结果；刷新页面会恢复 mock 默认值。真实系统落地时，这里应改为读取和保存策略配置接口。
